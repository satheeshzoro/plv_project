...
export const AppDataProvider = ({ children }) => {
  ...
  const checkAuth = useCallback(async () => {
    try {
      ...
      if (data.role === "ADMIN") {
        ...
        setAdminUser(null);
        setCurrentUser(null);
        setCurrentEditor({ id: data.id, name: data.full_name, email: data.email, role: data.role });
      } else {
        setIsAdminLoggedIn(false);
        setAdminUser(null);
        setCurrentEditor(null);
  ...
  useEffect(() => {
    ...
    fetchSettings();
    checkAuth();
  }, [fetchJournals, fetchTrendingArticles, fetchSettings, checkAuth]);
  ...
  const addEditor = async (editorData) => {
    try {
      await authFetch("/api/editors/", {
        method: "POST",
        body: JSON.stringify({
          email: editorData.email,
          full_name: editorData.name,
          pen_name: editorData.penName,
          country: editorData.country,
  ...
  const deleteEditor = (editorId) => {
    // API endpoint for deleting editor not provided in list
    // Keeping local optimistic update or removing functionality
    setEditors((prev) => prev.filter((e) => e.id !== editorId));
  };

  // Journal functions
  const addJournal = (journal) => {
    // Manual add journal not in API list. 
    // Assuming admin uses "Publish" flow from submissions.
    // Or implement if backend supports it.
    console.warn("Manual addJournal API not integrated");
  };

  const updateJournal = (journalId, updates) => {
    // Not in API list
  };
